class album_control_group: ctrlControlsGroupNoHScrollbars
{
	idc = 88000;
	x = X(0.1) + W(0.8/8);
	y = Y(0.1) + HI(0.0125);
	...
	class controls
	{
		...
		class copyright_control_group: title_control_group
		{
			idc = 170;
			x = W(0.115);
			y = HI(0.1) - H(0.02);
			...
			class controls
			{
				class copyright: ctrlActivePicture
				{
					idc = 175;
					text = "";
					x = 0;
		...
		class play_button: load_button
			...
			text = "Play Album";
			x = W(0.01);
			onButtonClick ="'ArmaSpotifyController' callExtension format['spotify:play_%1:%2', 'album', (_this#0) getVariable ['aasp_album_id','']];";
		};
	};
};
