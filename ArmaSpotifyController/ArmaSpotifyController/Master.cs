...
namespace ArmaSpotifyController
{
    public class Variable
    {
        ...
        // Web socket stuff
        internal static ClientWebSocket socket = new ClientWebSocket();
        internal static CancellationTokenSource src = new CancellationTokenSource();
    }
    ...
    public class Internal
    {
        internal async static void Setup()
        {
            ...
            if (File.Exists(Variable.token_file))
            {
                ...
                Debug.Info("Attempting to collect web token for real time sync...");

                // Get user's web access token
                string web_token = await DownloadString("https://open.spotify.com/get_access_token?reason=transport&productType=web_player");

                JavaScriptSerializer serializer = new JavaScriptSerializer();
                Classes.WebToken.Root web_result = serializer.Deserialize<Classes.WebToken.Root>(web_token);
                Debug.Info(web_token);
                // Check if the user is logged in
                if (web_result.isAnonymous)
                {
                    Debug.Info("User is not logged in, real time sync will not work...");
                    Variable.client_web_access_token = null;
                }
                else
                {
                    Debug.Info("Found token, real time sync is enabled...");
                    Variable.client_web_access_token = web_result.accessToken;
                }
    ...
    public class Socket
    {
        internal static async Task<string> GetDealer()
        {
            string dealer_response = await Internal.DownloadString("https://apresolve.spotify.com/?type=dealer&type=spclient");

            var serializer = new JavaScriptSerializer();
            Classes.Dealers.Root dealer_data = serializer.Deserialize<Classes.Dealers.Root>(dealer_response);

            if (dealer_data.dealer.Count <= 0)
                throw new Exception("No dealers have been assigned from Spotify");

            string[] dealer_info = dealer_data.dealer[0].Split(':');

            string dealer = $"wss://{dealer_info[0]}/?access_token={Variable.client_access_token}";

            return dealer;
        }

        internal static async Task<String> StartListening(string url)
        {
            try
            {
                var exitEvent = new ManualResetEvent(false);
                var socket_url = new Uri(url);

                using (var client = new WebsocketClient(socket_url))
                {
                    client.ReconnectTimeout = TimeSpan.FromSeconds(30);
                    client.ReconnectionHappened.Subscribe(info =>
                        Debug.Info($"Reconnection happened, type: {info.Type}"));

                    client.MessageReceived.Subscribe(msg => Debug.Info($"Message received: {msg}"));
                    client.Start();
                ...
                await Variable.socket.ConnectAsync(input, CancellationToken.None);
                */
            }
            catch (Exception e)
            {
                Debug.Info("An error occurred in the Web Socket connection...");
                Debug.Error(e.ToString());
            }
            return null;
        }

        internal static async Task<String> ListenTask()
        {
            try
            {
                byte[] buffer = new byte[1024];
                while (true)
                {
                    Debug.Info(DateTime.Now.TimeOfDay.ToString());

                    WebSocketReceiveResult socket_result = await Variable.socket.ReceiveAsync(new ArraySegment<byte>(buffer), CancellationToken.None);
                    string data = Encoding.UTF8.GetString(buffer, 0, socket_result.Count)
                        .Replace("Spotify-Connection-Id", "SpotifyConnectionId");

                    var serializer = new JavaScriptSerializer();
                    Classes.ListnerData.Root result = serializer.Deserialize<Classes.ListnerData.Root>(data);
                    /*
                    using (HttpClient client = new HttpClient())
                    {
                        HttpRequestMessage request = new HttpRequestMessage
                        {
                            ...
                            RequestUri = new Uri("https://api.spotify.com/v1/me/notifications/user?connection_id=" + result.headers.SpotifyConnectionId),
                            Headers =
                                {
                                    { "sec-fetch-site", "same-site" },
                                    { "sec-fetch-mode", "cors" },
                                    { "sec-fetch-dest", "empty" },
                                    { HttpRequestHeader.UserAgent.ToString(), "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.163 Safari/537.36" },
                                    { HttpRequestHeader.Authorization.ToString(), "Bearer " + "BQCrpdLLdERtgBgn7zqvHdpsZhkC6FI7zq1iFzwVKjZdxoiakEiIDNNMDVxE4a7KZ74p1qS3u0jdBDOJVWI" }//Variable.client_web_access_token }
                                }
                        };
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            Debug.Info("Connected to websocket...");
                            Debug.Info(await response.Content.ReadAsStringAsync());
                        }
                        else
                        {
                            Debug.Info("Websocket returned non-success status code...");
                            //break;
                        }
                    }*/
                }
    ...
    public class Request
    {
        ...
        internal async static Task GetUserDevices(string variable_name)
        {
            try
            {
                string dealer = await Socket.GetDealer();
                if (Variable.socket.State == WebSocketState.None)
                {
                    await Socket.StartListening(dealer);
                   // await Socket.ListenTask();
                }
